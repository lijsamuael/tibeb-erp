...
@frappe.whitelist()
def make_transport_request(source_name, target_doc=None):
    return get_mapped_transport_request(source_name, target_doc)

def get_mapped_transport_request(source_name, target_doc=None, ignore_permissions=False):
    def postprocess(source, target):
        target.flags.ignore_permissions = ignore_permissions
        set_missing_values_for_transport(source, target)

    def update_item(obj, target, source_parent):
        target.item_code = obj.item_code
        target.item_name = obj.item_name
        target.schedule_date = obj.schedule_date
        target.qty = obj.qty
        target.uom = obj.uom
    ...
    fields = {
        ...
        "Purchase Order Item": {
            "doctype": "Transport Request Item",
            "field_map": {
                "item_code": "item_code",
                "item_name": "item_name",
...
def set_missing_values_for_transport(source, target):
    # Add any additional default values or logic for the Transport Request
    target.request_date = frappe.utils.nowdate()
