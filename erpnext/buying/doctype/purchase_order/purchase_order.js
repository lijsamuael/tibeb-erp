...
erpnext.buying.PurchaseOrderController = class PurchaseOrderController extends erpnext.buying.BuyingController {
	setup() {
		this.frm.custom_make_buttons = {
			...
			'Transport Request': 'Request For Transport'
		}
	...
	refresh(doc, cdt, cdn) {
		...
		if(doc.docstatus == 1) {
			...
			if(doc.status != "Closed") {
				if (doc.status != "On Hold") {
					cur_frm.add_custom_button(__('Request For Transport'), this.make_transport_request, __('Create'));
	...
	make_transport_request() {
		frappe.model.open_mapped_doc({
			method: "erpnext.buying.doctype.purchase_order.purchase_order.make_transport_request",
			frm: cur_frm
		})
	}
