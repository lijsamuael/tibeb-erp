		...
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(
				...
				__("Tools")
						...
						// New button: Send Message to Telegram
			frm.add_custom_button(__("Send Telegram Message to Suppliers"), function() {
				if (!frm.doc.suppliers || frm.doc.suppliers.length === 0) {
					frappe.msgprint(__('No suppliers found.'));
					return;
				}
				...
				frm.doc.suppliers.forEach(supplier => {
					frappe.call({
						...
						callback: function(response) {
							if (response.message) {
								let phoneNumber = response.message.mobile_number;
								let message = `Request for Quotation: ${frm.doc.name}\nPlease review the attached RFQ document.`;
								...
								// Send API request
								frappe.call({
									method: "frappe.integration_office.request",
									args: {
										url: "https://tibeb-notification.onrender.com/send-message",
										method: "POST",
										headers: {
									...
									callback: function(res) {
										if (res.message) {
											frappe.msgprint(__('Message sent successfully to ' + phoneNumber));
										} else {
											frappe.msgprint(__('Failed to send message to ' + phoneNumber));
										}
									}
								});
							}
